...
def overview(db: Session = Depends(get_db)):
    ...
    for c in courses:
        enrolled_ids = set(
            db.scalars(select(Enrollment.user_id).where(Enrollment.course_id == c.id)).all()
        )
        enrolled = len(enrolled_ids)

        groups_count = db.scalar(
            select(func.count()).select_from(Group).where(Group.course_id == c.id)
        ) or 0

        members_in_groups = db.scalar(
            select(func.count())
        ...
        avg_group_size = float(members_in_groups) / groups_count if groups_count else 0.0
